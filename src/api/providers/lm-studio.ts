...
export class LmStudioHandler extends BaseProvider implements SingleCompletionHandler {
	...
	override async *createMessage(
		...
		try {
			...
			let results
			try {
				results = metadata?.signal
					? await this.client.chat.completions.create(params, { signal: metadata.signal })
			...
			throw new Error(
				"Please check the LM Studio developer logs to debug what went wrong. You may need to load the model with a larger context length to work with Mesa Code's prompts.",
			)
		}
	}
